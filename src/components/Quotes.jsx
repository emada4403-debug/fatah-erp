...
// ═══════════════════════════════════════════════════════════════════════════
export default function Quotes({ quotes, products, settings, onUpdate, onStartCreate, onStartEdit }) {
              ...
              <div className="bg-white rounded-2xl overflow-hidden shadow-2xl" style={{fontFamily:"'IBM Plex Sans',sans-serif"}}>
                ...
                {(wk.filter(r=>r.desc).length>0||tr_s.filter(r=>r.desc).length>0)&&<div className="px-10 pb-8">
                  ...
                  <div className={`grid gap-6 ${tr_s.filter(r=>r.desc).length&&wk.filter(r=>r.desc).length?'grid-cols-2':'grid-cols-1'}`}>
                    ...
                    {tr_s.filter(r=>r.desc).length > 0 && (
                      <div>
                        <h5 className="text-[9px] font-bold uppercase tracking-wider text-[#02273b] mb-3 flex items-center gap-2">
                          <span className="w-4 h-px bg-[#02273b] inline-block"/>
                          Transformation Surcharges
                        </h5>
                        <div className="rounded-xl border border-slate-100 overflow-hidden">
                          <table className="w-full border-collapse text-xs">
                            <thead>
                              <tr className="bg-slate-50 border-y border-slate-100 font-sans text-slate-400 uppercase text-[9px] font-bold">
                                <th className="py-2 px-3 text-left">Description</th>
                                <th className="py-2 px-3 text-right">Price (L.E)</th>
                              </tr>
                            </thead>
                            <tbody>
                              {tr_s.filter(r => r.desc).map((r, i) => (
                                <tr key={r.id || i} className={`border-b border-slate-100 last:border-0 ${i % 2 === 1 ? 'bg-slate-50' : ''}`}>
                                  <td className="py-2.5 px-3 text-slate-700 font-sans">{r.desc}</td>
                                  <td className="py-2.5 px-3 text-right font-semibold text-[#02273b]">{r.price}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                ...
                <div className="mx-10 mb-8 bg-slate-50 rounded-xl border border-slate-200 p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div><div className="text-[9px] uppercase tracking-wider text-slate-405 mb-1.5 font-bold">Payment Structure</div><p className="text-xs text-slate-700 leading-relaxed font-sans">{q.paymentTerms||'—'}</p></div>
                    <div><div className="text-[9px] uppercase tracking-wider text-slate-405 mb-1.5 font-bold">Delivery Timeline</div><p className="text-xs text-slate-700 leading-relaxed font-sans"><strong>{q.deliveryDays||'14-21'} Business Days</strong></p></div>
                    <div><div className="text-[9px] uppercase tracking-wider text-slate-405 mb-1.5 font-bold">Quote Validity</div><p className="text-xs text-slate-700 leading-relaxed font-sans">Valid until <strong>{fmtDateEN(q.validUntil)}</strong></p></div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-3 gap-6 flex-1">
                    {['Sales Management', 'Contract Control', 'General Manager'].map(s => (
                      <div key={s} className="text-center">
                        ...
                        <div className="text-[9px] font-bold uppercase tracking-wider text-slate-400">{s}</div>
                      </div>
                    ))}
                  </div>
                  <div className="rounded-xl p-6 min-w-[240px] shadow-lg flex-shrink-0" style={{background:'#02273b'}}>
                    <div className="space-y-2 text-sm text-right">
                      <div className="flex justify-between text-slate-300 gap-6"><span>Subtotal:</span><span>{fmtEN(tots.subtotal)} L.E</span></div>
                      {(q.discountPct||0) > 0 && <div className="flex justify-between text-rose-300 gap-6"><span>Discount ({q.discountPct}%):</span><span>-{fmtEN(tots.discountAmt)} L.E</span></div>}
                      <div className="flex justify-between text-slate-300 gap-6"><span>VAT ({q.taxPct??14}%):</span><span>{fmtEN(tots.taxAmt)} L.E</span></div>
                      <div className="h-px my-1" style={{background:'rgba(255,255,255,0.15)'}}/>
                      <div className="flex justify-between items-end font-sans font-bold text-base text-emerald-400 gap-6"><span>Grand Total:</span><span>{fmtEN(tots.total)} L.E</span></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-t border-slate-100 px-10 py-4 flex justify-between items-center bg-slate-50">
                  ...
                  <div className="text-xs text-slate-400 font-sans">© {new Date().getFullYear()} {cNameEn} — All Rights Reserved.</div>
                </div>
              </div>
            </div>
          </div>
