...
export const CostEngine = {
  calculate(product) {
    ...
    const rawBreakdown = (ce.rawMaterials || []).map(item => {
      ...
      rawTotal += total;
      return { ...item, unitCost, total, matName: mat ? mat.name : 'غير محدد' };
    });
    ...
    const finalPrice = (product.sellingPrice !== undefined && product.sellingPrice !== null && product.sellingPrice > 0)
      ? product.sellingPrice
      : totalCost * (1 + (product.marginPct || 0) / 100);

    const margin = (product.sellingPrice !== undefined && product.sellingPrice !== null && product.sellingPrice > 0)
      ? (totalCost > 0 ? ((product.sellingPrice - totalCost) / totalCost * 100) : 0)
      : (product.marginPct || 0);
  ...
  calcQuoteItem(product, qty, discountPct = 0) {
    ...
    return { ...result, unitPrice, qty, subtotal, discountPct, discountAmt, total };
  },
};
